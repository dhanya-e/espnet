...
# data
datadir_google=/home/dhanya/PhD/Datasets/Google_Indic/kn/  #Google Indic
datadir_air=/home/dhanya/PhD/Datasets/AIR_Kannada_Dataset_graphemes/Kannada_Dataset/  #AIR Kannada
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    echo "stage -1: Data Download and Data preparation"
    #mkdir -p ${datadir_google}
    #local/download_and_untar.sh ${datadir} ${data_url}
    python local/data_prep_google.py ${datadir_google} data/local/data_google/
    python local/data_prep_air.py ${datadir_air} data/local/data_air/
    bash local/ka_data_prep.sh data/local/data_google  data/local/data_air ${trans_type} || exit 1
fi

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    bash local/ka_format_data.sh
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in test train dev; do
        ...
        make_fbank.sh --cmd "$train_cmd" --nj 8 --write_utt2num_frames true \
